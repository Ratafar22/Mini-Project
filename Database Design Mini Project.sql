...
/* Answer: the schema contains a table named inventory non-normalized which has 4,581 rows and 13 columns. The table includes information on films and stores.
Also, there is a lot of duplicated information in the datasets which needs to be normalised. */
...
/* Question 3:
Based on your answer from question #2, create a new schema with the tables you think will best serve this dataset. */
...
-- The inventory_id in the inventory table is used as the primary key while the store_id and the film_id are used as the foreign keys and Non- NULL constraint was added to all columns
-- The Film table: The film_id was used as the primary key and a non-NULL constraint was added to all columns
-- Store table: the store_id was used as the primary key and a non-NULL constraint was added to the columns. */
